...
def get_snowflake_session():
    """Get the active Snowflake session with error handling"""
    try:
...
def execute_semantic_query(dimensions: List[str], metrics: List[str], filters: Optional[str] = None, limit: int = 1000, semantic_view: str = "snowflake_monitoring_semantic") -> Optional[pd.DataFrame]:
    """Execute a semantic view query using Snowpark"""
    ...
    try:
        ...
        # Try a fallback simple query to test connection
        try:
            fallback_query = "SELECT CURRENT_USER() as USER, CURRENT_DATABASE() as DB"
            fallback_df = session.sql(fallback_query).to_pandas()
            st.info("✅ Session is working, but semantic view query failed")
            st.write("Current context:", fallback_df)
        except Exception as fallback_e:
            st.error(f"Even fallback query failed: {fallback_e}")
...
def generate_insights(df: pd.DataFrame, query: str) -> List[str]:
    ...
    try:
        ...
        insights.append(f"⚠️ Error generating some insights: {str(e)}")
...
def create_visualization(df: pd.DataFrame, dimensions: List[str], metrics: List[str]) -> go.Figure:
    ...
    try:
        ...
        # Scatter plot for two metrics
            ...
            if metrics[0] in df.columns and metrics[1] in df.columns:
        ...
        return go.Figure().add_annotation(text=f"Visualization error: {str(e)}", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
    ...
    with st.sidebar:
        ...
        if session:
            ...
            try:
                ...
                st.warning(f"Could not get context: {e}")
        else:
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        if 'avg' in prompt.lower() or 'average' in prompt.lower() or 'timing' in prompt.lower():
                            ...
                            elif 'AVG_USER_EXECUTION_TIME' in df.columns:
                        ...
                        error_msg = "❌ No data found for your query. This might be because:\n"
                        error_msg += "• The semantic views don't exist yet\n"
                        error_msg += "• The dimensions/metrics are not available\n"
                        error_msg += "• Try rephrasing your question\n"
                        error_msg += "• Check the available metrics in the sidebar"
                        st.error(error_msg)
                    ...
                    error_msg += "• A simpler question\n"
                    error_msg += "• Check if semantic views are set up\n"
                    error_msg += "• Use the quick actions in the sidebar"
                    st.error(error_msg)
...
def dashboard_view():
    ...
    try:
        ...
        if overview_data is not None and not overview_data.empty:
            ...
            st.warning("⚠️ No data available for dashboard. Make sure semantic views are configured.")
